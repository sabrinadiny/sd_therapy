...
class JobBase:
    ...
    def shall_run(self, *args: Any, **kwargs: Any) -> bool:
        return True

    def run_private(self) -> None:
        return self.run(*self.args, **self.kwargs)

    def shall_run_private(self) -> bool:
        return self.shall_run(*self.args, **self.kwargs)

    def make_hash_from_args(self) -> str:
        args_dict = {'args': self.args, 'kwargs': self.kwargs}
...
class JobItem:
    ...
    def __init__(self, job_class_or_file: str | Path | type[Job], *args: Any, **kwargs: Any) -> None:
        if isinstance(job_class_or_file, (six.string_types, Path, str)):
    ...
    def _init_from_class(self, job_class: type[Job], *args: Any, **kwargs: Any) -> None:
        tmp_module = inspect.getmodule(job_class)
        if tmp_module is None:
            raise ValueError('Job class must be in a module')
        self.job_module = tmp_module.__name__
        if self.job_module == '__main__':
            self.job_module = Path(Path(sys.argv[0]).name).stem
    ...
    def _init_from_json(self, f_name: str | Path) -> None:
...
class AutomaticFilenameJob(Job, ABC):
    ...
    base_data_folder = ''
    job_data_folder = ''
    exclude_kwargs_from_filename: list[str] = []
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        self.subject_id = kwargs.get('subject_id', 'dummy')

        super().__init__(*args, **kwargs)

    @classmethod
    def get_full_data_folder(cls) -> Path:
        """
    ...
    def output_filename(self) -> str:
        ...
        for key, val in self._kwargs.items():
            ...
            f_name_list.append(f'{key}_{str(val)}')
